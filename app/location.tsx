...
export default function LocationScreen() {
  ...
  const searchAddresses = async (query: string) => {
    ...
    try {
      ...
      const url = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(query)}, Ecuador&format=json&limit=10&addressdetails=1`;
      ...
      const filteredResults = data.filter((result: any) => {
        ...
        const region = display.includes('ambato') || display.includes('tungurahua');
        return region;
      });
      
      // Si hay resultados filtrados, usarlos; si no, mostrar todos
      setSearchResults(filteredResults.length > 0 ? filteredResults.slice(0, 5) : data.slice(0, 5));
      
      console.log('Resultados de búsqueda:', data);
    } catch (error) {
      console.error('Error buscando direcciones:', error);
      setSearchResults([]);
      Alert.alert('Error', 'No se pudo buscar direcciones. Verifica tu conexión a internet.');
    } finally {
      setSearching(false);
    }
  };
      ...
      >
        <SafeAreaView style={styles.modalContainer}>
          <View style={styles.modalHeader}>
            ...
            <ThemedText style={styles.modalTitle}>Selecciona tu ubicación</ThemedText>
            <TouchableOpacity onPress={handleConfirmLocation} disabled={!selectedAddress}>
